...
func (u *userPostgresRepo) GetUserByID(ctx context.Context, ID uint) (*model.User, error) {
	user := &model.User{}
	err := u.postgresCLI.WithContext(ctx).Preload("Articles").Preload("Comments").Preload("Likes.Article.Comments").Preload("Likes.Article.Categories").Preload("Bookmarks.Article").First(user, ID).Error
	if err != nil {
